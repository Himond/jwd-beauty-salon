...
public class AddCouponCommand implements Command {
    ...
    public Router execute(HttpServletRequest request) throws ServletException, IOException {
        ...
        try {
            Optional<Coupon> currentCoupon = service.findCouponByCode(code);
            if (currentCoupon.isPresent()){
                Coupon coupon = currentCoupon.get();
                cart.setCoupon(coupon);
                session.setAttribute(CART, cart);
            }else {
                request.getSession().setAttribute(COUPON_NOT_FOUND, MessageManager.valueOf(local.toUpperCase(Locale.ROOT)).getMessage(COUPON_NOT_FOUND_PATH));
            }
